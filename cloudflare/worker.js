...
export default {
  async fetch(request,env){
    ...
    if(request.method==='GET'&&url.pathname==='/health'){
      return json({ok:true,model:MODEL,budgetGuard:!!env.USAGE,apiKeyConfigured:!!env.OPENAI_API_KEY,accessCodeConfigured:!!env.ACCESS_CODE,publicMode:env.REQUIRE_ACCESS_CODE!=='true'},200,headers);
    }
    ...
    if(env.REQUIRE_ACCESS_CODE==='true'){
      if(!env.ACCESS_CODE)return json({error:'ACCESS_CODE is required but not configured.'},503,headers);
      if(body.accessCode!==env.ACCESS_CODE)return json({error:'Incorrect AI access code.'},401,headers);
    }
    if(typeof body.image!=='string'||!body.image.startsWith('data:image/'))return json({error:'A profile screenshot is required.'},400,headers);
    if(body.image.length>12_000_000)return json({error:'Screenshot is too large.'},413,headers);
