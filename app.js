(() => {
  ...
  async function handleProfile(file){
    if(!file)return;
    try{
      const {img,url}=await load(file);S.jobToken++;S.scanning=false;
  ...
  $('retryBuddyBtn').addEventListener('click',regenerateCharacters);
