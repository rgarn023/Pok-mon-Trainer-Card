(() => {
  ...
  function loadLockedBackground(team){
    fetch(`assets/locked/${team}.b64?v=18`,{cache:'no-store'})
      ...
      .catch(()=>{const img=new Image();img.onload=()=>{backgrounds[team]=img;S.drawFront?.();S.drawBack?.();};img.src=`assets/${team}-bg.svg?v=18`;});
  }
  ...
  function contain(g,img,x,y,w,h,z=1,px=0,py=0){const iw=img?.naturalWidth||img?.width||0,ih=img?.naturalHeight||img?.height||0;if(!iw||!ih)return;const scale=Math.min(w/iw,h/ih)*z,nw=iw*scale,nh=ih*scale;g.drawImage(img,x+(w-nw)/2+px*w*.18,y+(h-nh)/2+py*h*.18,nw,nh);}
  ...
  function drawSubjects(g){
    ...
    if(S.generatedSubject||S.combinedCutout){
      contain(g,S.generatedSubject||S.combinedCutout,45,145,510,445,1.08,0,0);
    }else{
      // Locked composition: buddy large behind-left, trainer foreground right-center.
      if(S.buddyCutout)contain(g,S.buddyCutout,45,145,360,430,1.12,-.08,-.02);
      if(S.trainerCutout)contain(g,S.trainerCutout,275,190,270,365,1.08,.04,.01);
    }
    g.restore();
  }
  ...
  S.drawFront=()=>{
    const c=$('cardCanvas');if(!c)return;if(c.width!==W*SCALE||c.height!==H*SCALE){c.width=W*SCALE;c.height=H*SCALE;}const g=c.getContext('2d');g.clearRect(0,0,c.width,c.height);g.save();g.scale(SCALE,SCALE);const team=S.team||'valor';drawBackground(g,team);drawAnimatedEffect(g,team);drawHeader(g,team);drawSubjects(g);drawStats(g,team);g.restore();
  };
  ...
  S.drawBack=()=>{
    ...
    g.fillStyle='#fff';panelPath(g,157,212,286,286,16);g.fill();if(S.qrCrop)contain(g,S.qrCrop,174,229,252,252);else{g.fillStyle='#151922';g.font='800 13px system-ui';g.fillText('UPLOAD TRAINER CODE',300,353);g.fillStyle='#657080';g.font='600 9px system-ui';g.fillText('QR preview appears here',300,375);}
