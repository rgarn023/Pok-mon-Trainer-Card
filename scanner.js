(() => {
  ...
  async function scanStats(){
    ...
    for(const [id,label,r,mode,list,rules] of jobs){ $('scanDetail').textContent=label; const a=S.crop(S.sourceImage,...r,6),b=S.crop(S.sourceImage,...r,6,mode); const reads=[await S.read(a,'7',list),await S.read(b,'7',list),await S.read(b,'13',list)]; if(id==='startDate'){ const value=cleanDate(reads.map(x=>x.text).join(' ')); if(value){$(id).value=value;S.setState(id,bestRead(reads).confidence>=50?'ok':'review')}else S.setState(id,'review'); } else { const pick=chooseNumber(reads,rules||{}); if(pick){$(id).value=id==='level'?pick.digits:formatDigits(pick.digits);S.setState(id,pick.confidence>=50?'ok':'review')} else S.setState(id,'review'); } }
  }

  function makeSubjectCrops(img){
    const iw=img.naturalWidth, ih=img.naturalHeight;
    const trainerRect={x:.41,y:.11,w:.29,h:.43};
    const buddyRect={x:.14,y:.03,w:.58,h:.46};
    const trainer=document.createElement('canvas'); trainer.width=Math.round(iw*trainerRect.w); trainer.height=Math.round(ih*trainerRect.h); trainer.getContext('2d').drawImage(img,iw*trainerRect.x,ih*trainerRect.y,iw*trainerRect.w,ih*trainerRect.h,0,0,trainer.width,trainer.height);
    const buddy=document.createElement('canvas'); buddy.width=Math.round(iw*buddyRect.w); buddy.height=Math.round(ih*buddyRect.h); buddy.getContext('2d').drawImage(img,iw*buddyRect.x,ih*buddyRect.y,iw*buddyRect.w,ih*buddyRect.h,0,0,buddy.width,buddy.height);
    S.trainerCrop=trainer; S.buddyCrop=buddy; S.trainerCutout=null; S.buddyCutout=null;
    $('trainerEmpty').classList.add('hidden'); $('buddyEmpty').classList.add('hidden'); $('retryTrainerBtn').disabled=false; $('retryBuddyBtn').disabled=false;
    $('trainerCutoutState').textContent='Prepared'; $('buddyCutoutState').textContent='Prepared'; $('extractStatus').textContent='Prepared';
  ...
  function cleanAlpha(srcCanvas,minAreaRatio=.02){
    ...
    const keep=new Uint8Array(id+1); for(let i=1;i<=id;i++) if(areas[i]>=Math.max(80,maxArea*minAreaRatio)) keep[i]=1;
    ...
    g.putImageData(im,0,0); if(maxX<0) return null; const pad=Math.round(Math.max(w,h)*.02),x0=Math.max(0,minX-pad),y0=Math.max(0,minY-pad),x1=Math.min(w,maxX+pad),y1=Math.min(h,maxY+pad),o=document.createElement('canvas'); o.width=x1-x0+1; o.height=y1-y0+1; o.getContext('2d').drawImage(c,x0,y0,o.width,o.height,0,0,o.width,o.height); return o;
  }
  async function runRemoveBg(blob,device,progressId){ if(!S.removeBg){ const m=await import('https://cdn.jsdelivr.net/npm/@imgly/background-removal@1.7.0/+esm'); S.removeBg=m.removeBackground||m.default; } return S.removeBg(blob,{device,model:'isnet_quint8',progress:(key,current,total)=>{ if(total>0) $(progressId).textContent=`${Math.round(current/total*100)}%`; },output:{format:'image/png',quality:1,type:'foreground'}}) }
  async function extractOne(kind){
    const crop=kind==='trainer'?S.trainerCrop:S.buddyCrop; const stateEl=$(kind+'CutoutState'); if(!crop) return; stateEl.textContent='Extracting…'; $('extractStatus').textContent=`Extracting ${kind}…`;
    try{
      const p=resize(crop,1024),blob=await new Promise(r=>p.toBlob(r,'image/png')); let out; const gpu=!!navigator.gpu; try{ out=await runRemoveBg(blob,gpu?'gpu':'cpu', kind+'CutoutState'); }catch(e){ if(!gpu) throw e; out=await runRemoveBg(blob,'cpu', kind+'CutoutState'); }
      const url=URL.createObjectURL(out),im=new Image(); await new Promise((r,j)=>{ im.onload=r; im.onerror=j; im.src=url });
      const cleaned=cleanAlpha(im, kind==='trainer' ? .03 : .018); URL.revokeObjectURL(url); if(!cleaned) throw new Error('empty cutout');
      if(kind==='trainer') S.trainerCutout=cleaned; else S.buddyCutout=cleaned;
      stateEl.textContent='Ready'; $('extractStatus').textContent='Ready'; $('extractHelp').textContent='Trainer and buddy are extracted separately. The card front will only render transparent cutouts.';
      S.drawPreviews(); S.drawFront();
    }catch(e){ console.warn(e); if(kind==='trainer') S.trainerCutout=null; else S.buddyCutout=null; stateEl.textContent='Retry'; $('extractStatus').textContent='Needs review'; $('extractHelp').textContent='Automatic cutout failed for one or more subjects. The final card will not use a screenshot fallback.'; S.drawPreviews(); S.drawFront(); }
  }
  S.extractTrainerCutout=()=>extractOne('trainer');
  S.extractBuddyCutout=()=>extractOne('buddy');
  S.extractAllCutouts=async()=>{ if(!S.trainerCrop || !S.buddyCrop) return; await extractOne('buddy'); await extractOne('trainer'); };

  S.scanProfile=async()=>{ if(!S.sourceImage||S.scanning) return; S.scanning=true; $('scanCard').classList.remove('hidden'); $('scanPct').textContent='0%'; try{ S.setTeam(S.detectTeam(S.sourceImage),true); await scanIdentity(); await scanStats(); makeSubjectCrops(S.sourceImage); await S.extractAllCutouts(); S.updateQuality(); S.toast('Profile scan complete. Review anything marked Review.'); } catch(e){ console.error(e); S.toast('Some profile fields need manual review.'); } finally { S.scanning=false; $('scanCard').classList.add('hidden'); S.drawFront(); S.drawBack(); } };
