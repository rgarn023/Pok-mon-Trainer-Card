...
const defaults=()=>({trainer:{cx:.5,cy:.44,w:.62,h:.72},buddy:{cx:.5,cy:.45,r:.31},looking:{cx:.5,cy:.5,r:.30},favorite:{cx:.5,cy:.5,r:.30}});
...
function sendPointer(canvas,type,pointerId,clientX,clientY){
  const init={bubbles:true,cancelable:true,pointerId,pointerType:'touch',isPrimary:true,clientX,clientY,button:0,buttons:type==='pointerup'?0:1,pressure:type==='pointerup'?0:.5};
  const ev=new PointerEvent(type,init);
  if(type==='pointerdown'&&canvas.setPointerCapture){const old=canvas.setPointerCapture;try{canvas.setPointerCapture=()=>{};canvas.dispatchEvent(ev);}finally{canvas.setPointerCapture=old;}}
  else canvas.dispatchEvent(ev);
}
function canvasClient(canvas,x,y){const r=canvas.getBoundingClientRect();return{x:r.left+x/canvas.width*r.width,y:r.top+y/canvas.height*r.height};}
function gesture(kind,action){
  const canvas=$(kind+'CropCanvas');if(!canvas||canvas.width<=1||canvas.height<=1)return;
  const s=state[kind],cw=canvas.width,ch=canvas.height,min=Math.min(cw,ch),step=.045,pid=31;
  let sx,sy,tx,ty;
  if(kind==='trainer'){
    const x=(s.cx-s.w/2)*cw,y=(s.cy-s.h/2)*ch,w=s.w*cw,h=s.h*ch;
    if(action==='left'||action==='right'||action==='up'||action==='down'){
      sx=s.cx*cw;sy=s.cy*ch;tx=sx+(action==='left'?-step*cw:action==='right'?step*cw:0);ty=sy+(action==='up'?-step*ch:action==='down'?step*ch:0);
      s.cx=clamp(s.cx+(tx-sx)/cw,s.w/2,1-s.w/2);s.cy=clamp(s.cy+(ty-sy)/ch,s.h/2,1-s.h/2);
    }else if(action==='wider'||action==='narrower'){
      sx=x+w;sy=y+h/2;tx=sx+(action==='wider'?step*cw:-step*cw);ty=sy;
      s.w=clamp(s.w+2*(tx-sx)/cw,.18,.96);s.cx=clamp(s.cx,s.w/2,1-s.w/2);
    }else if(action==='taller'||action==='shorter'){
      sx=x+w/2;sy=y+h;tx=sx;ty=sy+(action==='taller'?step*ch:-step*ch);
      s.h=clamp(s.h+2*(ty-sy)/ch,.20,.96);s.cy=clamp(s.cy,s.h/2,1-s.h/2);
    }else return;
  }else{
    const r=s.r*min,cx=s.cx*cw,cy=s.cy*ch;
    if(action==='left'||action==='right'||action==='up'||action==='down'){
      sx=cx;sy=cy;tx=sx+(action==='left'?-step*cw:action==='right'?step*cw:0);ty=sy+(action==='up'?-step*ch:action==='down'?step*ch:0);
      s.cx=clamp(s.cx+(tx-sx)/cw,.05,.95);s.cy=clamp(s.cy+(ty-sy)/ch,.05,.95);const rx=s.r*min/cw,ry=s.r*min/ch;s.cx=clamp(s.cx,rx,1-rx);s.cy=clamp(s.cy,ry,1-ry);
    }else if(action==='larger'||action==='smaller'){
      sx=cx+r;sy=cy;tx=sx+(action==='larger'?step*min:-step*min);ty=sy;s.r=clamp(s.r+(tx-sx)/min,.08,.48);const rx=s.r*min/cw,ry=s.r*min/ch;s.cx=clamp(s.cx,rx,1-rx);s.cy=clamp(s.cy,ry,1-ry);
    }else return;
  }
  const a=canvasClient(canvas,sx,sy),b=canvasClient(canvas,tx,ty);sendPointer(canvas,'pointerdown',pid,a.x,a.y);sendPointer(canvas,'pointermove',pid,b.x,b.y);sendPointer(canvas,'pointerup',pid,b.x,b.y);
  requestAnimationFrame(()=>{$(kind+'ApplyCrop')?.click();renderSlot(kind);});
}
function buildAdjustControls(kind){
  const canvas=$(kind+'CropCanvas'),wrap=canvas?.closest('.cropCanvasWrap');if(!canvas||!wrap||$(kind+'CropAdjust'))return;
  canvas.style.pointerEvents='none';
  const block=document.createElement('div');block.id=kind+'CropAdjust';block.className='cropAdjustBlock';
  const sizeButtons=kind==='trainer'
    ? '<button data-act="narrower">Narrower</button><button data-act="wider">Wider</button><button data-act="shorter">Shorter</button><button data-act="taller">Taller</button>'
    : '<button data-act="smaller">Smaller</button><button data-act="larger">Larger</button>';
  block.innerHTML=`<div class="cropAdjustHead"><b>Adjust crop</b><span>The screenshot stays scrollable. Use these buttons to move or resize the crop.</span></div><div class="cropAdjustGrid"><div class="movePad"><button data-act="up" aria-label="Move crop up">↑</button><button data-act="left" aria-label="Move crop left">←</button><button class="center" type="button" disabled>MOVE</button><button data-act="right" aria-label="Move crop right">→</button><button data-act="down" aria-label="Move crop down">↓</button></div><div class="sizePad">${sizeButtons}</div></div><div class="cropScrollNote">Swipe directly on the screenshot above to scroll the page at any time.</div>`;
  wrap.insertAdjacentElement('afterend',block);
  block.querySelectorAll('button[data-act]').forEach(b=>b.addEventListener('click',()=>gesture(kind,b.dataset.act)));
}
